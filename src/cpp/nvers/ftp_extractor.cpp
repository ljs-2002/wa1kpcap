/**
 * ftp_extractor.cpp  ——  FTP 控制通道元特征提取（JSON Lines）
 * 用法：./ftp_extractor -r <file.pcap> [-w ftp.log] [-v]
 */
#include "ftp_flow.h"
...
struct FtpFlowKey {
    ...
    uint8_t  proto;
    bool operator==(const FtpFlowKey& o) const noexcept {
        return src_ip==o.src_ip && dst_ip==o.dst_ip &&
               src_port==o.src_port && dst_port==o.dst_port && proto==o.proto;
    }
    FtpFlowKey canonical() const noexcept {
        if (is_ftp_port(dst_port)) return *this;
        if (is_ftp_port(src_port)) return {dst_ip, src_ip, dst_port, src_port, proto};
        if (src_ip < dst_ip) return *this;
        if (src_ip > dst_ip) return {dst_ip, src_ip, dst_port, src_port, proto};
        return src_port <= dst_port ? *this
               : FtpFlowKey{dst_ip, src_ip, dst_port, src_port, proto};
    }
    std::string id() const {
        char buf[96];
...
struct FtpFlowKeyHash {
    size_t operator()(const FtpFlowKey& k) const noexcept {
...
static std::unordered_map<FtpFlowKey, FtpFlowRecord, FtpFlowKeyHash> g_flows;
...
static void handle_tcp(uint32_t sip, uint32_t dip, uint16_t sp, uint16_t dp,
    ...
    if (!is_ftp_port(sp) && !is_ftp_port(dp)) return;
    if (plen <= 0) return;

    FtpFlowKey raw{sip, dip, sp, dp, 6};
    FtpFlowKey can = raw.canonical();
    bool is_client = is_ftp_port(dp);
...
wa1kpcap::nvers::ExtractResult wa1kpcap::nvers::run_ftp(const ExtractConfig& cfg) {
    ExtractResult res;
    ...
    const char* out = cfg.output_path.empty() ? "ftp.log" : cfg.output_path.c_str();
    g_out = fopen(out, "w");
