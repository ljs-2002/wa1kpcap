/**
 * cicext_extractor.cpp  ——  CIC 扩展版特征提取器（78 + 192 列）
 *
 * ┌───────────────────────────────────────────────────────────┐
 * │  架构（多生产者-多消费者流水线）                          │
 * │                                                           │
 ...
 *
 * 用法：
 *   ./cicext_extractor -r <pcap>   [-n N] [-j <线程数>] [-w <csv>]   # -n 0=全流(默认)
 *   ./cicext_extractor -i <网卡>   [-n N] [-j <线程数>] [-w <csv>]
 */

#include "cicext_flow.h"
...
static inline void emit_record(CicExtRecord& rec) {
    rec.finalize();
    {
        std::lock_guard<std::mutex> lk(g_out_mutex);
        rec.emit(*g_out);
    }
    rec.base.emitted = true;
...
private:
    ...
    // 流表：canonical FlowKey → CicExtRecord（无锁，仅本 Worker 访问）
    std::unordered_map<FlowKey, CicExtRecord, FlowKeyHash> flows_;
    ...
    void process_slot(const PktSlot& slot) {
        ...
        if (it == flows_.end()) {
            // 新流
            CicExtRecord rec;
            rec.base.fwd_key = key;
            rec.base.n_limit = n_limit_;
            ...
            snprintf(rec.base.flow_id, sizeof(rec.base.flow_id),
                     "%s_%u_%s_%u_%u",
            ...
            is_fwd = (key == it->second.base.fwd_key);
        }
        ...
        if (it->second.base.done && !it->second.base.emitted) {
    ...
    void age_flows(double now_ts, bool force = false) {
        ...
        for (auto& [k, rec] : flows_) {
            if (rec.base.emitted) { expired.push_back(k); continue; }
            double idle = now_ts - rec.base.last_ts;
            double dur  = rec.base.last_ts - rec.base.start_ts;
            double timeout = (rec.base.fwd_key.proto == IPPROTO_TCP)
...
static void print_usage(const char* prog) {
    fprintf(stderr,
        ...
        "  -w <csv>    输出 CSV（默认 features_ext.csv，- 表示 stdout）\n"
        "  -h          显示帮助\n",
        prog, prog);
}

wa1kpcap::nvers::ExtractResult wa1kpcap::nvers::run_cicext(const ExtractConfig& cfg) {
    ExtractResult res;
    if (cfg.pcap_path.empty()) { res.exit_code = 1; res.message = "pcap_path required"; return res; }

    std::string output_file = cfg.output_path.empty() ? "features_ext.csv" : cfg.output_path;
    ...
    write_cicext_csv_header(*out_ptr);
...
int main(int argc, char* argv[]) {
    ...
    std::string output_file = "features_ext.csv";
    int  n_limit   = CIC_DEFAULT_N;
    ...
    return wa1kpcap::nvers::run_cicext(cfg).exit_code;
}
#endif
